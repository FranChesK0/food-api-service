...
from .response import Item, Menu

router = APIRouter(prefix="/menu", tags=["Restaurant Menu"])
...
async def get_menu(restaurant_id: int, session: SessionDep) -> Menu:
    ...
    for category_id, category_items in by_category.items():
        ...
        items.append(Item(category=category, items=category_items))

    return Menu(restaurant_id=restaurant_id, items=items)
